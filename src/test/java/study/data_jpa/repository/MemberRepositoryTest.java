...
class MemberRepositoryTest {
    ...
    public void bulkUpdate() {
        ...
        // em.flush(); jpql 호출전에 flush 호출하고 jpql 실행됨(commit)
        // em.clear();

        List<Member> result = memberRepository.findByUsername("member5");
        Member member5 = result.get(0);
        System.out.println("member5 = " + member5); // 증가하는 쿼리 실행했는데 왜 증가안됐지?
    ...
    public void findMemberLazy() {
        ...
        // when N(10) + 1 문제 -> 패치조인으로 해결
        // select Member 1
        // List<Member> members = memberRepository.findAll(); // proxy 객체(Team)
        List<Member> members = memberRepository.findMemberFetchJoin(); // join쿼리로 team객체도 세팅함
